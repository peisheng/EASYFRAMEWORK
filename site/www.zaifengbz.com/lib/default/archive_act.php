...
class archive_act extends act {
    ...
    function show_action() {
        ...
        foreach ($linkwords as $linkword) {
            ...
            /*$words[] = $linkword['linkword'];
            $links[] = $link;
            $times[] = $linkword['linktimes'];
            session::set($linkword['linkword'],0);*/
        }
        //$content = preg_replace("/&(\w+);/i", "@>\\1<@", $content);
        //$content = preg_replace("/(^|>|&)([^<|;]+)(?=<|$)/sUe", "_highlight('\\2', \$words, \$links, '\\1',\$times)", $content);
        //$content = preg_replace("/(<a(.*))-\]-(.*)-\[-(\/a>)/isU", '\\1>\\3<\\4', $content);
        //$content = str_replace("/@/a>([a-zA-Z]+)<a@/i", "&\\1;", $content);
		...
		if(is_array($this->view->archive['pics']) && !empty($this->view->archive['pics'])){
			foreach ($this->view->archive['pics'] as $k => $v){
				if(strtolower(substr($v,0,7)) == 'http://'){
					...
					$this->view->archive['pics'][$k] = config::get('base_url').'/'.$v;
				}
			}
		}
		//$this->view->archive['pics'] = unserialize($this->view->archive['pics']);
