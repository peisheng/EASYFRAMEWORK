...
namespace Wpf.Utils
{
    ...
    {
        public static string GetPingyin(string chineseWorld)
        {
            return ChinesePY.GetPinYin(chineseWorld);
        }
        /// <summary>
        /// 利用正则表达式验证信息
        /// </summary>
        ...
        {
            return source != null && source != "" && System.Text.RegularExpressions.Regex.IsMatch(source, pattern);
        }
        ...
        /// <returns></returns>
        public static List<string> GetTemplateKey(string templateContent)
        {
            string keyPartten = @"{{(?<key>\w{2,})}}";
            MatchCollection mc = Regex.Matches(templateContent, keyPartten);
        ...
        /// <returns>加密后的字任串</returns>
        public static string GetEncryptDES(string inputString)
        {
          return  DesEncryption.EncryptDES(inputString);
        }

        /// <summary>
        /// 对加密的字符串进行解密
        /// </summary>
        /// <param name="decryptString">加密了的字符串</param>
        /// <returns></returns>
        public static string GetDecryptDES(string decryptString)
        {
            return DesEncryption.DecryptDES(decryptString);
        }
        ...
        /// <returns></returns>
        public static string GetCnNum(string num)
        {
