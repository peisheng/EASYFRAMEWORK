namespace l3v5y.Wordpress
{
    ...
    {
        public event WordpressEventHandler get_author_posts_complete;
        public void get_author_posts(int id, int count = 0, int page = 0, string post_type = null)
        {
            WordpressEventArgs args = new WordpressEventArgs(WordpressDataType.PostList, HttpStatusCode.OK, true);
            string str = url + "?json=get_author_posts&id=" + id;
            if (count != 0)
            {
            ...
            Helper.HttpWebRequestDownload((asynchronousResult) =>
            {
                ...
                Deployment.Current.Dispatcher.BeginInvoke(new Action(() =>
                {
                    try
                    {
                        get_author_posts_complete.Invoke(pl, args);
                    }
                    catch
                    {
                    }
        ...
        public void get_author_posts(string slug, int count = 0, int page = 0, string post_type = null)
        {
            WordpressEventArgs args = new WordpressEventArgs(WordpressDataType.PostList, HttpStatusCode.OK, true);
            string str = url + "?json=get_author_posts&slug=" + slug;
            if (count != 0)
            {
                str += "&count=" + count.ToString();
            }
