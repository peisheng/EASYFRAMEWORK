<?php
$pagename_template = "taoproduct"; //这个字段一定要跟页面名称对上。 
...
if($TaoConfig["applicationdata"]["GJconfig"]["listlinktype"]!="localitem"){
	echo("<script language=\"javascript\">location.href='".$rootroad."/gotourl.php?iid=".$param["item_id"]."';</script>");	
	exit;
}

$totalResults = 0;

$taoke = GetTaoProduct($param["item_id"]);
$vpro = $taoke[0];

$vpro = GetTaoProductZhePrice($vpro);
 
 
$TaoapiCat = GetTaoCates(array_merge(array("cids"=>$vpro["cid"])));

$TaoShop = GetTaoShop($vpro[nick]);
