...
class indexAction extends Action {
    ...
    public function index() {
		...
		if (file_exists($config_install)) {
			exit("您好，已经安装过了。如果您需要重新安装，请直接删除 \"data/install.lock\"即可，这个文件在您的data目录下。  <br> 默认后台目录是 http://您的域名/admin 目录。");
		}
    ...
    public function show_process() {
		...
		$pagetext = $pagetext.'$tongjidaima="'.$tongjidaima.'";  //统计脚本'."\n";
		...
		$pagetext = $pagetext.'$paipai_uid="'.$paipai_uid.'";  //AppSecret'."\n";
		$pagetext = $pagetext.'$paipai_uin="'.$paipai_uin.'";  //AppSecret'."\n";
		$pagetext = $pagetext.'$paipai_appOAuthID="'.$paipai_appOAuthID.'";  //AppSecret'."\n";
		$pagetext = $pagetext.'$paipai_appOAuthkey="'.$paipai_appOAuthkey.'";  //AppSecret'."\n";
		$pagetext = $pagetext.'$paipai_accessToken="'.$paipai_accessToken.'";  //AppSecret'."\n";
		$pagetext = $pagetext.'$templatefolder="'.$templatefolder.'";  //templatefolder'."\n";
		$pagetext = $pagetext.'define(\'ROOT\',dirname(__FILE__)."/");'."\n";
		
		//写入到缓存中.
		file_put_contents ( "./data/configdata.php", "<?php \n".$pagetext."\n?>");
