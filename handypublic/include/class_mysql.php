...
class dbstuff {
	...
	function query($sql, $type = '') {
		...
		if(DEBUG) {
			...
			$sqlstarttime = number_format(($mtime[1] + $mtime[0] - $TaoConfig['supe_starttime']), 6) * 1000;
		}
		...
		if(DEBUG) {
			$mtime = explode(' ', microtime());
			$sqlendttime = number_format(($mtime[1] + $mtime[0] - $TaoConfig['supe_starttime']), 6) * 1000;
			...
			$TaoConfig['debug_query'][] = array('sql'=>$sql, 'time'=>$sqltime, 'info'=>$info, 'explain'=>$explain);
		}
