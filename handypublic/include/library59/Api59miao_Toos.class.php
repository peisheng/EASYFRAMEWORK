...
*/		
		...
		//访问服务
        self::$Taoapi_Util->fetch($url);
        $file_contents = self::$Taoapi_Util->results;
		...
		static public function getXmlData ($strXml) {
			...
			$strXml = str_replace("encoding=\"GBK\"","encoding=\"UTF-8\"",$strXml);
			$strXml = Newiconv("GBK","UTF-8",$strXml);

			if ($pos) {
				$xmlCode=simplexml_load_string($strXml);
