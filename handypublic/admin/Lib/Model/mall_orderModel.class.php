<?php
class mall_orderModel extends RelationModel{
	...
	public function get_mall_order($id){
		$mod=D('mall_order');
	...
	function get_list($pagesize=20){
		...
		$list=$mod->where($where)->order("created desc")->limit($p->firstRow.','.$p->listRows)->select();
