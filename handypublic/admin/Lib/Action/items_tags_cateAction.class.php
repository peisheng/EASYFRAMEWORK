...
class items_tags_cateAction extends baseAction
{
	...
	public function index()
	{
		...
		$big_menu = array('javascript:art.dialog({id:\'add\',iframe:\'?m=items_tags_cate&a=add&cate_id='.$cate_id.'\', title:\'添加标签 - '.$pcate_info['name'].' &gt; '.$cate_info['name'].'\', width:\'750\', height:\'430\', lock:true}, function(){var d = art.dialog({id:\'add\'}).data.iframe;var form = d.document.getElementById(\'dosubmit\');form.click();return false;}, function(){art.dialog({id:\'add\'}).close()});void(0);', '添加标签');
		$page = $p->show();
