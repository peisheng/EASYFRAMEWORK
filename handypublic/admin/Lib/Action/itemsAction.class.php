...
class itemsAction extends baseAction {
    ...
    public function index() {
        ...
        foreach ($items_list as $k => $val) {
			...
			if(substr($items_list[$k]['item_key'],0,6)=="taobao"){
				$items_list[$k]['site']="<font color=#FF0000>淘宝</font>";
			}
			if(substr($items_list[$k]['item_key'],0,6)=="paipai"){
				$items_list[$k]['site']="<font color=#CC00FF>拍拍</font>";
			}
	...
	public function getarrayTaoBao(&$count){
		...
		$param["keyword"] = htmlspecialchars(urldecode(var_request("keyword","推荐")));
		...
		//过滤标题
			for($i = 0; $i < count($taobaokeItem); $i++) {
				...
				if($taobaokeItem[$i]["price_with_rate"] != "") $taobaokeItem[$i]["price"].=" 折扣价：".$taobaokeItem[$i]["price_with_rate"];
    ...
    public function taoapiadd() {
		...
		$keyword = var_request("keyword","推荐");
    ...
    public function taoapiaddok() {
		...
		$taobaokeItem = $this->getarrayTaoBao($count);
			...
			$this->success(L(' 采集成功，添加'.$addnum."个商品，更新".$updatenum."个商品,失败".$errornum."个商品。"));
